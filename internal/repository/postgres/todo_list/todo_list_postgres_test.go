	...
	tests := []struct {
	...
	}{
		{
			...
			mock: func() {
				...
				mock.ExpectQuery("SELECT (.+) FROM todo_lists t1 INNER JOIN users_lists t2 on (.+) WHERE (.+)").
...
func TestTodoListPostgres_Delete(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			name: "Ok",
			mock: func() {
				mock.ExpectExec("DELETE FROM todo_lists t1 USING users_lists t2 WHERE (.+)").
...
func TestTodoListPostgres_Update(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			name: "OK_AllFields",
			mock: func() {
				mock.ExpectExec("UPDATE todo_lists t1 SET (.+) FROM users_lists t2 WHERE (.+)").
